...
let VOD = {};// 二级的单个数据
let TABS = [];// 二级的自定义线路列表 如: TABS=['码上放生','码上放生2']
    ...
    if (p === '*') {
        ...
        if (p.content) {
            try {
                ...
                vod.vod_content = prefix + _pdfh(html, p1[0]).replace(/\n|\t/g, '').trim();
            }
            catch (e) { }
        }
        if (p.img) {
        ...
        vod.vod_play_from = playFrom.map(item => prefix + item).join(vod_play_from);
