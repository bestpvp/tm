...
async function home(filter) {
    ...
    for (const type of data) {
        let typeName = prefix + type.nav_name;
        if (typeName == prefix + '推荐') continue;
...
async function detail(id) {
    ...
    let vod = {
        ...
        vod_content: prefix + data.vod_content.trim(),
    };
    let playlist = {};
    ...
    vod.vod_play_from = _.keys(playlist).map(key => prefix + key).join('$$$');
